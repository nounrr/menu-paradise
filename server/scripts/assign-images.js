...
try {
  ...
  for (const [index, dish] of dishes.entries()) {
    ...
    if (!imageFile) {
      missing += 1;
      console.warn(`Image manquante pour #${index + 1} "${dish.name_fr}": ${imageNumber}.webp`);
      continue;
    }

    const uploadName = path.basename(imageFile);
  ...
  console.log(`Premiere image utilisee: ${options.start}.webp`);
} finally {
  await pool.end();
}
